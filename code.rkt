...
(define (playlist-duration songLengths repeatCounts)
  (define (playlist-duration-helper songLengths repeatCounts totalSongDuration)
    (cond ((and (null? songLengths) (null? repeatCounts)) totalSongDuration)
          ((null? songLengths) (playlist-duration-helper songLengths (cdr repeatCounts) (append totalSongDuration (list 0))))
          ((null? repeatCounts) (playlist-duration-helper (cdr songLengths) repeatCounts (append totalSongDuration (list (car songLengths)))))
  ...
  (playlist-duration-helper songLengths repeatCounts '()))
...
(define (cumulative-playlist-duration songDurations)
  ...
  (cond ((null? songDurations) songDurations)
        (else (cumulative-playlist-duration-helper (cdr songDurations) '() (car songDurations)))
        ))
...
(define (flatten-subtract-triple lst)
  ...
  (map (lambda (x) (* 3 (- x 1))) (flatten-lst lst)))
...
(define (interleave-duplicates lst)
  ...
  (interleave-duplicates-helper lst '()))
...
(define (nested-sum lst)
  ...
  (nested-sum-helper (flatten-lst lst) 0))
...
(playlist-duration '(200 180 240) '(2 3 1 1 1));
|#
